...
class Chatbot:
    def __init__(self):
        self.Filtropreco = ProdutoFiltro()
        ...
        self.database_connection_str = f'mysql+pymysql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}/{DB_DATABASE}'

    def conectar_banco_dados(self):
        engine = create_engine(self.database_connection_str)
        df_produtos = pd.read_sql_table('produtos', con=engine)
        return df_produtos

    def criar_pasta_train_data(self):
    ...
    def create_training_data(self, df_produtos):
        train_data = {"data": []}

        # Normalizar os nomes dos produtos no DataFrame
        df_produtos['nome_normalizado'] = df_produtos['nome'].apply(self.normalizar_texto)
    ...
    def salvar_dados_treinamento(self, train_data, data_dir):
    ...
    def comparar_perguntas(self, pergunta_usuario, perguntas_treinamento):
    ...
    def lidar_com_saudacoes(self, pergunta):
        saudacoes = [
    ...
    def responder_com_base_no_produto(self, pergunta_usuario, df_produtos):
        pergunta_normalizada = self.remover_acentos(self.preprocessar_texto(pergunta_usuario))
        
        for _, produto in df_produtos.iterrows():
            nome = self.remover_acentos(produto['nome']).lower()
    ...
    def fazer_pergunta(self, train_data, pergunta_usuario, df_produtos):
        try:
            ...
            similarity_scores = self.comparar_perguntas(pergunta_usuario, perguntas_treinamento)
            
            if len(similarity_scores) == 0:
                resposta_base_produto = self.responder_com_base_no_produto(pergunta_usuario, df_produtos)
    ...
    def main(self):
        df_produtos = self.conectar_banco_dados()

        if df_produtos.empty:
            print("O DataFrame de produtos está vazio. Verifique a conexão com o banco de dados e a tabela 'produtos'.")
        else:
            train_data = self.create_training_data(df_produtos)
            data_dir = self.criar_pasta_train_data()
            self.salvar_dados_treinamento(train_data, data_dir)
            ...
            while True:
                ...
                resposta_saudacao = self.lidar_com_saudacoes(pergunta_usuario)
                if resposta_saudacao:
                    print(resposta_saudacao)
                    continue
                
                # Primeiro, verifique a filtragem de preços
                resposta_maximo = self.Filtropreco.filtrar_produtos_por_valor_maximo(pergunta_usuario)
                if resposta_maximo is not None and not resposta_maximo.empty:
                    print(resposta_maximo)
                    continue

                resposta_minimo = self.Filtropreco.filtrar_produtos_por_valor_minimo(pergunta_usuario)
                if resposta_minimo is not None and not resposta_minimo.empty:
                    print(resposta_minimo)
                    continue

                resposta_intervalo = self.Filtropreco.filtrar_produtos_por_intervalo(pergunta_usuario)
                ...
                resposta = self.fazer_pergunta(train_data, pergunta_usuario, df_produtos)
                print(resposta)
